...
exports.handler = async (event) => {
  ...
  if (stripeEvent.type === 'checkout.session.completed') {
    ...
    if (tipo === 'socio_mensual') {
      // Hacerse socio: solo actualizar estado de suscripción, sin acceso a curso
    ...
    } else if (tipo === 'vela_perpetua') {
      // Activar la vela perpetua tras confirmar el pago
      const { velaId } = session.metadata || {};
      if (velaId) {
        await sb.from('velas').update({
          pagado: true,
          activa: true
        }).eq('id', velaId);
      }
  ...
  if (stripeEvent.type === 'customer.subscription.deleted') {
    ...
    if (sub) {
      // Solo revocar acceso si era un curso real (no para socios)
      if (sub.curso_id && sub.curso_id !== 'socio_mensual') {
